...
+ (void)showImage:(UIImage *)image status:(NSString *)string {
    [[SVProgressHUD sharedView] showImage:image status:string duration:3.0];
}
...
//        CGSize stringSize = [string sizeWithFont:self.stringLabel.font constrainedToSize:CGSizeMake(200, 300)];
        ...
        CGRect rect=[string boundingRectWithSize:CGSizeMake(_width, 2000) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.stringLabel.font} context:nil];
        CGSize stringSize=CGSizeMake(CGRectGetWidth(rect), CGRectGetHeight(rect));
        ...
        if(hudHeight > 100) {
            labelRect = CGRectMake(12, 33, hudWidth, stringHeight);
            hudWidth+=24;
        } else {
            hudWidth+=24;  
            labelRect = CGRectMake(0, 33, hudWidth, stringHeight);
        }
    }
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [UIView animateWithDuration:0.15
                         ...
                         animations:^{	
                             ...
                             [self removeFromSuperview];//张飞蓬改此，显示消失以后有残留的阴影
                             self.alpha = 0;
                         }
...
//        hudView.backgroundColor = [UIColor colorWithRed:52/255.0 green:170/255.0 blue:56/255.0 alpha:0.8];
//		hudView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.8];
        hudView.backgroundColor=[[UIColor darkTextColor] colorWithAlphaComponent:0.8];
