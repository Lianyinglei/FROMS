...
- (void)drawFace:(CGContextRef)context
{
    switch (_innerBackgroundStyle)
    {
        ...
        break;
#pragma  mark ----- 表盘的小圆背景色----------
        case WMGaugeViewInnerBackgroundStyleFlat:
        {
            #define EXTERNAL_RING_RADIUS    0.14
            ...
            CGContextSetFillColorWithColor(context, kUIColorFromRGB(0x59c895).CGColor);
...
- (void)drawText:(CGContextRef)context
{
    ...
    [attrStr drawAtPoint:CGPointMake(0.5 - fontWidth.width / 2.0, 0.68)];
}
...
- (void)drawScale:(CGContextRef)context
{
 ...
 #pragma mark  ------color为表盘单位的颜色-------
              color = [UIColor blackColor];
            CGContextSetStrokeColorWithColor(context, color.CGColor);
            CGContextSetLineWidth(context, _scaleDivisionsWidth);
            CGContextSetShadow(context, CGSizeMake(0.05, 0.05), _showScaleShadow?1.0:0.0);
