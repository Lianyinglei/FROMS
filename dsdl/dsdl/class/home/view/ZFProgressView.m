...
#pragma mark - setter and getter methond---进度条的位置----
    ...
    if(!_progressLabel)
    {
        _progressLabel = [[UILabel alloc] initWithFrame:CGRectMake((self.bounds.size.width - 100)/2, (self.bounds.size.height - 100)/2, 100, 100)];
          if (self.isHome) {
                  ...
                  self.progressLabel.font = [UIFont systemFontOfSize:26 weight:0.4];
                self.progressLabel.frame = CGRectMake((self.bounds.size.width - 100)/2, (self.bounds.size.height - 100)/2, 100, 100);
          }else{
          
                _progressLabel.frame = CGRectMake(self.width*0.5-50, -10, 100, 30);
                _progressLabel.frame  = CGRectZero;
                self.progressLabel.font = [UIFont systemFontOfSize:13 weight:0.2];
...
-(void)numberAnimation
{
    ...
    float sumSteps =  _Percentage /self.timeDuration *_sumSteps;
     // NSLog(@"----sumSteps---%f",self.sumSteps);
    if (_sumSteps >= self.timeDuration) {
     ...
     // NSLog(@"sumSteps---%f--------%f",sumSteps,self.timeDuration);
