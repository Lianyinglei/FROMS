...
- (void)contentHeaderTap:(UITapGestureRecognizer *)recognizer {
    ...
    NSString *columnStr = [NSString stringWithFormat:@"%ld_%ld", (long)section, (long)column];
    ...
    if (section == -1) {
        ...
        for (int i = 0; i < rows; i++) {
            ...
            NSString *str = [NSString stringWithFormat:@"%ld_%ld", (long)iPath.section, (long)iPath.row];
            [columnSortedTapFlags setObject:[NSNumber numberWithLong:columnFlag] forKey:str];
...
- (void)singleHeaderClick:(NSIndexPath *)indexPath {
    ...
    [columnSortedTapFlags setObject:[NSNumber numberWithLong:columnFlag] forKey:columnStr];
